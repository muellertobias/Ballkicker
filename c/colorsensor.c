...
static unsigned char sensor_mode[4];
 ...
 * @param [in] port -> NXT_PORT_S1, NXT_PORT_S2, NXT_PORT_S3, NXT_PORT_S4
 */
static void colorsensor_light_full(int port)
{
	volatile int dummy;
...
{	
	...
	sensor_mode[port] = address; // save sensor mode
...
int colorsensor_light_get(int port)
{
	...
	if (sensor_mode[port] == LIGHTSENSOR_FULL)
	{
		colorsensor_light_full(port); // it also gets rgb data
	}
 ...
 */
void colorsensor_rgb_get(int port, int rgb[3])
{
	colorsensor_color_get(port);
	rgb[0] = rgb_buf[0];
	rgb[1] = rgb_buf[1];
	rgb[2] = rgb_buf[2];
}
 ...
 * @param [in] port -> NXT_PORT_S1, NXT_PORT_S2, NXT_PORT_S3, NXT_PORT_S4
 * @return WHITE, RED, GREEN, DARK_BLUE, YELLO, BLACK, LIGHT_BLUE
 */
...
{   
    ...
    dummy = colorsensor_light_get(port); // get rgb
    red = rgb_buf[0];
    green = rgb_buf[1];
    blue = rgb_buf[2];
